...
class _EditProduct extends State<EditProduct> {
  ...
  //Variables para la edición del producto
  String productTitle;
  ...
  loadData() async {
    ...
    setState(() {
      productTitle = sharedPreferences.get("description_product_edit");
      descriptionController.text = sharedPreferences.get("description_product_edit");
      precioController.text = sharedPreferences.get("price_product_edit");
      identificadorController.text = sharedPreferences.get("identifier_product_edit");
    });
      ...
      if (jsonData['resultado'][0]['error'] == 0) {
        ...
        if (_clients.length == 1) {
          setState(() {
            ...
            clientSelected = true;
            loadTypeProducts(sharedPreferences.get("client_product_edit"));
          });
        } else {
          setState(() {
            uniqueClient = "";
            ...
            someClients = true;
            progressDialog.dismiss();
            clientValue = sharedPreferences.get("client_product_edit");
            loadTypeProducts(clientValue);
          });
        }
      } else {
  ...
  saveInfo() async {
    if (formKey.currentState.validate()) {
      ...
      var pnUsuario = sharedPreferences.get("user");
      var pnClave = sharedPreferences.get("pass");
      var pnProduct = sharedPreferences.get("product_edit");
      print(clientValue);
      Map json = {
        "autenticacion": {"pn_usuario": pnUsuario, "pn_clave": pnClave},
        "parametros": {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData['resultado'][0]['error'] == 0) {
          progressDialog.dismiss();
          print("Sin error");
          gFunct.showModalDialog("Edición exitosa",
              "El producto fue modificado con éxito.", context);
        } else {
          gFunct.showModalDialog(
              "Error al obtener información",
              "Hubo un error al solicitar sus opciones, intentelo en unos minutos",
